...
Generation 的 optional `platform` 是 format `1` compatible expansion. 当前新 Writer 必须写入 `openai`; 缺少该字段的既有 Generation 继续合法, 由 read model 按 tool identity 推断或显示 Unknown, 不执行 Archive migration.
